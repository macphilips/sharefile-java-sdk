...
public final class Filter {
   ...
   *
   * <p>Note: OData v3 uses {@code substringof(needle, haystack)} — the search value comes first,
  ...
  private static Filter combine(String operator, Filter... filters) {
    ...
    if (filters.length < 2) {
      throw new IllegalArgumentException("'%s' requires at least two filters".formatted(operator));
    }
   ...
   *
   * <ul>
   *   <li>Strings → single-quoted with internal quotes escaped
   *   <li>Booleans → {@code true}/{@code false}
   *   <li>Numbers → numeric literal
   *   <li>Other → {@code toString()} without quoting
   * </ul>
   */
