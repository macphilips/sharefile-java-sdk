...
final class HttpRequests {
   ...
   * @return a new HTTP request
   */
  static HttpTransport.HttpRequest streaming(
      String method, URI uri, InputStream inputStream, long size, Duration timeout) {
    Objects.requireNonNull(inputStream, "inputStream must not be null");
    var headers = new LinkedHashMap<String, String>();
    headers.put("Accept", "application/json");
    return new DefaultHttpRequest(
        method, uri, Collections.unmodifiableMap(headers), inputStream, size, timeout);
  }
  ...
  static HttpTransport.HttpRequest noBody(String method, URI uri, Duration timeout) {
    ...
    headers.put("Accept", "application/json");
    return new DefaultHttpRequest(
        method, uri, Collections.unmodifiableMap(headers), null, -1, timeout);
  }
   ...
   * @return a new HTTP request
   */
  static HttpTransport.HttpRequest formEncoded(
      String method, URI uri, byte[] body, Duration timeout) {
    Objects.requireNonNull(body, "body must not be null");
    var headers = new LinkedHashMap<String, String>();
