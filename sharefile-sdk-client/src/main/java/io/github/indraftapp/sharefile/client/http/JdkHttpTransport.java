 ...
 *
 * <ul>
 *   <li>Response bodies are always streamed via {@code BodyHandlers.ofInputStream()} — never
 *       buffered in full
 *   <li>Upload bodies are streamed from the caller-provided {@link InputStream} while preserving a
 *       fixed {@code Content-Length} when the caller provides one
 *   <li>Redirects are set to {@code NEVER} — the SDK controls redirect handling to prevent leaking
 *       authorization headers to storage-zone hosts
 *   <li>{@code IOException} and {@code InterruptedException} are wrapped in {@link
...
public final class JdkHttpTransport implements HttpTransport {
   ...
   *
   * <p>This constructor is primarily for testing. The caller is responsible for configuring the
   * client appropriately (redirect policy, timeouts, etc.).
   *
  ...
  public HttpResponse execute(HttpRequest request) throws ShareFileNetworkException {
    ...
    try {
      ...
      return new JdkHttpResponse(
          jdkResponse, deadlineNanos, request.timeout(), request.method(), request.uri());
    } catch (IOException e) {
  ...
  private java.net.http.HttpRequest toJdkRequest(HttpRequest request) {
    var builder =
        java.net.http.HttpRequest.newBuilder().uri(request.uri()).timeout(request.timeout());
                ...
                stream -> {
                  ...
                  if (contentLength.isPresent()) {
                    return BodyPublishers.fromPublisher(
                        new InputStreamBodyPublisher(stream), contentLength.getAsLong());
                  } else {
  ...
  private static final class JdkHttpResponse implements HttpResponse {
    ...
    JdkHttpResponse(
      ...
      response
          ...
          .map()
          .forEach(
              (name, values) -> normalized.put(name.toLowerCase(Locale.ROOT), List.copyOf(values)));
      this.headers = Map.copyOf(normalized);
      this.bodyStream =
          new TimeoutAwareInputStream(response.body(), deadlineNanos, timeout, requestDescription);
    }
    ...
    public byte[] bodyBytes() throws ShareFileNetworkException {
      try (var stream = bodyStream) {
        ...
        throw new ShareFileNetworkException(
            "Failed to read response body for %s: %s".formatted(requestDescription, e.getMessage()),
            e);
      }
    }
  ...
  private static final class TimeoutAwareInputStream extends InputStream {
    ...
    private TimeoutAwareInputStream(
      ...
      this.timeoutMessage =
          "%s timed out while reading the response body after %s"
              .formatted(requestDescription, timeout);
      long delayNanos = Math.max(0L, deadlineNanos - System.nanoTime());
      this.timeoutFuture =
          RESPONSE_TIMEOUT_EXECUTOR.schedule(this::timeoutClose, delayNanos, TimeUnit.NANOSECONDS);
    }
  ...
  private static final class InputStreamBodyPublisher implements Flow.Publisher<ByteBuffer> {
    ...
    public void subscribe(Flow.Subscriber<? super ByteBuffer> subscriber) {
      ...
      if (!subscribed.compareAndSet(false, true)) {
        subscriber.onError(
            new IllegalStateException("Request body publisher only supports one subscriber"));
        return;
      }
  ...
  private static final class InputStreamSubscription implements Flow.Subscription {
    ...
    private InputStreamSubscription(
        Flow.Subscriber<? super ByteBuffer> subscriber, InputStream inputStream) {
