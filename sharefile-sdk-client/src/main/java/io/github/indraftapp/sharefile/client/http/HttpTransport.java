 ...
 *
 * <p>Implementations must support streaming request and response bodies. File transfers can be
 * multi-GB — the transport must never require buffering entire upload/download bodies as byte
 * arrays.
 *
 * <p>The default implementation is {@link JdkHttpTransport}, which uses {@code
...
public interface HttpTransport {
  ...
  interface HttpRequest {
     ...
     *
     * <p>Returns empty if the body length is unknown (chunked transfer encoding). Returns empty if
     * there is no body.
     *
    ...
    /**
     * The request timeout. This is the total time allowed for the request to complete, including
   ...
   *
   * <p>Callers must close the {@link #bodyStream()} when done to release the underlying connection.
   */
  interface HttpResponse extends AutoCloseable {
    ...
    /**
     * Response headers. Header names are case-insensitive per HTTP spec; implementations should
     * normalize to lowercase.
     */
    Map<String, List<String>> headers();

    /**
     * The response body as a stream. Always returns a valid stream, even for empty bodies (returns
     * an empty stream).
     *
     * <p>For file downloads, consume this stream directly to avoid buffering the entire response in
     * memory.
     */
     ...
     *
     * <p>Use this only for JSON API responses (typically &lt; 1 MB). For file downloads, use {@link
     * #bodyStream()} instead.
     *
